...
# - Prefer ../docs/gem_registry.json (repo layout)
# - Fallback to local gem_registry.json (packaged app layout)
_HERE = Path(__file__).resolve().parent
_CANDIDATES = [
    (_HERE / "../docs/gem_registry.json").resolve(),
    (_HERE / "gem_registry.json").resolve(),
]

REG_PATH = next((p for p in _CANDIDATES if p.exists()), _CANDIDATES[-1])
...
def _backfill_defaults(name: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # JSON uses the key "class" (reserved word in Python). Internally we use "cls".
    if "cls" not in data and "class" in data:
        data["cls"] = data.pop("class")
    data.setdefault("cls", "Unknown")
    ...
    # Ensure mandatory fields exist (fail early with clear error if missing)
    # We intentionally do NOT invent values for core optical fields.
    required = ["A", "B", "delta_n", "anisotropic", "cls", "foundation", "symbol", "color", "label"]
    missing = [k for k in required if k not in data]
    if missing:
        raise ValueError(f"Gem '{name}' missing required keys: {missing}")
...
def load_registry() -> Dict[str, Gem]:
    ...
    for name, payload in raw.items():
        ...
        # Drop any unknown keys defensively (future-proofing)
        allowed = set(Gem.__dataclass_fields__.keys()) - {"name"}
        clean = {k: v for k, v in payload.items() if k in allowed}
        gems[name] = Gem(name=name, **clean)
